...
const registryName = 'product-rules';
...
const startMarker = '<!-- PRODUCT_RULES_SUMMARY_START -->';
const endMarker = '<!-- PRODUCT_RULES_SUMMARY_END -->';
...
if (!Array.isArray(registry.rules)) throw new Error('Product rules registry must contain a rules array.');
const visibleRules = registry.rules.filter((rule) => rule.hubVisible !== false);
const generated = visibleRules.map((rule) => {
  requireHubText(rule, ['hubTitle', 'hubSummary', 'hubScope']);
  return `          <article class="ops-panel" data-search-item>
            <div class="label-row"><span class="pill">${escapeHtml(rule.hubScope)}</span></div>
