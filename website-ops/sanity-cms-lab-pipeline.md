...
- ↗ [Открыть отчёт по Sanity](sanity-guide.html) — человеческий разбор
  CMS-слоя, наполнения данных и переиспользования на лендингах.
- ⬇ [Скачать skill `/sanity-content`](sanity-content.md) — инструкция агента
  для наполнения Sanity данными без изменения схемы.
...
## Наполнение данных через `/sanity-content`

Разработчик добавил отдельный skill `/sanity-content` для наполнения Sanity
данными: кейсами, отзывами, спикерами, FAQ, цитатами, принципами философии,
финальными фразами и продуктами. Полный человеческий отчёт:
[`sanity-guide.html`](sanity-guide.html), текст skill:
[`sanity-content.md`](sanity-content.md).

Главная граница: skill меняет только данные, а не схему. Он может создавать или
обновлять документы, загружать фото, собирать references и наполнять существующие
блоки. Он не должен добавлять, переименовывать или удалять поля в
`studio/schemaTypes/`, менять validation, заводить новые типы документов или
перестраивать блоки без отдельной schema-задачи.

Для записи агенту лучше работать внутри `ai-mindset-website`, потому что там
есть `studio/schemaTypes/`, `.env`, примеры `scripts/seed-*.mjs` и актуальные
GROQ-запросы. Если запускать агента на запись вне этого repo, ему придется
дублировать контракт схемы, а рассинхрон повышает риск испортить данные.

Безопасный сценарий записи:

1. Разобрать намерение: сущность, блок или целая лаба; тип документа; источник
   данных; наличие фото; что делать с дублями.
2. Провалидировать обязательные поля, форматы и references до записи.
3. Проверить дубли по id-полю (`title` или `name`), взять реальный `_id`
   найденного документа и использовать `patch` для частичных правок.
4. Перед записью показать dry-run: создать / обновить / пропустить, затем
   получить подтверждение.
5. Не печатать `SANITY_API_TOKEN`, не удалять документы и не добавлять npm
   dependencies.

Чтение Sanity для сторонних страниц проще: опубликованный dataset читается без
токена через Sanity client и GROQ. Это публичный сайт в JSON-форме; черновики,
запись и удаление без write-токена недоступны. Для браузерного чтения с внешнего
лендинга нужно добавить origin в Sanity CORS allowlist.
...
- `reports/sanity-guide.html`
- `.claude/commands/sanity-content.md`
