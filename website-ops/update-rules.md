...
| Surikat Sonya design review bot | `Surikat Sonya` | Nightly candidate scan and daytime DM review delivery when review windows are known | Local watcher, manual `/review`, Cloud Run webhook, or future schedule after night design output | Telegram screenshot cards, 0-10 ratings, feedback registry events, Sonya-to-Vasily handoff, optional Vertex AI / Cloud Storage media path |
...
- Addressed questions, discussion, and bot-flow critique may use LLM only for final short wording after deterministic routing. The LLM must not accept tasks, create Linear issues, claim screenshots/dashboard files, or invent current QA status.
...
- She sends only candidates with `sonyaScore > 5/10`; candidates without a Gemini score, manifest score, or explicit README self-rating are not sent.
...
- Cloud Run webhook mode exposes `POST /telegram`, `GET /health`, and protected `POST /review-queue`; do not run Telegram polling and webhook mode at the same time.
...
- Cloud Run currently supports webhook, Vertex AI, and Cloud Storage media upload; durable owner/dispatch/registry/handoff state still needs a Cloud Storage or Firestore backend before Cloud Run becomes the only source of truth.
