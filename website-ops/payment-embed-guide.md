...
`pay_contact_entered` — отдельная цель восстановления лида: в её параметрах Метрики хранится
введённый Telegram/телефон. У остальных технических `pay_event` значения полей формы отсутствуют.

| Общая цель | Что означает | Состояние в текущем исходнике на 21 июля 2026 |
| --- | --- | --- |
| `pay_page_loaded` | Открылась `/pay` с конкретным `product` | Событие отправляется, цель создана |
| `pay_contact_entered` | Введён Telegram или телефон | Событие отправляется, цель создана |
| `pay_checkout_clicked` | Нажата кнопка оплаты после заполнения формы | Событие отправляется, цель создана |
| `pay_provider_redirected` | Получена ссылка провайдера и начат переход | Событие отправляется, цель создана |
| `pay_error` | Запуск оплаты завершился ошибкой до перехода | Событие отправляется, цель создана |
| `pay_completed` | Провайдер подтвердил успешную оплату | Событие отправляется, цель создана |

[Открыть общий счётчик лабораторий и оплаты в Яндекс Метрике](https://metrica.yandex.com/overview?id=106857835)
...
### Как разделять аналитику разных лабораторий и тарифов

`/pay` — одна общая страница для всех лабораторий. Для каждого нового тарифа нужен новый
`product_code`, но **не нужен новый счётчик Метрики и не нужен отдельный набор целей**.

Параметр `product` в ссылке определяет тариф. Страница оплаты добавляет этот код в параметры
платёжных событий как `product_code`. Поэтому ссылки
`/pay?product=s26_main` и `/pay?product=ain3_main` отправляют события в один счётчик `106857835`,
но их можно разложить по разным продуктовым отчётам.

Дашборд не является отдельным местом приёма событий. Это сохранённый отчёт или сегмент внутри
счётчика. Событие не нужно динамически «переключать» между дашбордами по `product`: оно один раз
попадает в общий счётчик, а нужный дашборд отбирает только свои `product_code`.

Для каждой новой лаборатории:

1. Получите и проверьте отдельный `product_code` для каждого её тарифа.
2. Подключите к лендингу общий счётчик `106857835`, если его там ещё нет. Иначе Метрика увидит
   только `/pay`, но не просмотры лендинга и клики по его CTA.
3. Передавайте точный `product_code` в ссылке или `data-product`. Клик конкретного тарифа должен
   сохранять тот же код, который затем использует `/pay`.
4. В Метрике создайте отдельный сохранённый отчёт/дашборд лаборатории и включите в его сегмент все
   её тарифные коды. Например, отчёт AI Native включает `ain3_mainearly`, `ain3_main`,
   `ain3_teamearly` и `ain3_team`; отчёт S26 — `s26_main`.
5. Используйте общий набор этапов: просмотр лендинга, просмотр тарифа, клик CTA на лендинге,
   `pay_page_loaded`, `pay_contact_entered`, `pay_checkout_clicked`, `pay_provider_redirected`,
   `pay_completed` и `pay_error`. Не создавайте копии этих целей с названием продукта.
6. Перед публикацией проверьте тестовый проход и убедитесь, что он появился в общем счётчике и в
   сегменте нужной лаборатории.

Отдельный счётчик создаётся только для действительно изолированного проекта: другой владелец или
набор доступов, отдельные юридические требования либо запрет смешивать аудитории. Обычный новый
тариф или новая лаборатория таким исключением не являются.
